...
const RandomQuestionsScreen = ({ route, navigation }) => {
  ...
  const [similarPairIndices, setSimilarPairIndices] = useState([]);

  const CHATGPT_API_KEY = "sk-wpX97RgQFsVtP3Y0jjYNRCJH7IaVsR3Vva3YFycLXkT3BlbkFJPSy3jNqwqYOe8wAKt9FG2EBUrdZiKnryCVHhkZuYcA"; // Replace with your OpenAI API key
  ...
  const fetchRandomQuestions = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:6000/api/random-questionsss', {
        params: { categories: categories.join(',') },
      });
      setQuestions(response.data.questions);
      setSimilarPairIndices(response.data.similarPairIndices); // <-- Add this line
    } catch (error) {
...
const generateAndSaveGPTQuestions = async () => {
  ...
  const handleSelectAnswer = (option) => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      console.log("my selected answers",updatedAnswers)
      navigation.navigate('AnswerScreen', { selectedAnswers: updatedAnswers, questions, similarPairIndices });    }
  };
